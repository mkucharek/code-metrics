...
**ALL code changes MUST pass these checks before marking a task as Done:**
...
### ✅ Use Custom Error Types (NEVER generic Error)

**ALWAYS use domain-specific error types from `src/domain/errors`:**

#### Available Error Types:

**GitHub API Errors:**
- `GitHubAuthenticationError` - Invalid token, auth failed
- `GitHubResourceNotFoundError` - Repository, PR, or resource not found
- `GitHubRateLimitError` - API rate limit exceeded
- `GitHubApiError` - Generic GitHub API failures

**Validation Errors:**
- `ValidationError` - Data validation failures (Zod, type guards)

**Database Errors:**
- `DatabaseError` - Database operation failures
- `DatabaseConnectionError` - Database connection issues
- `DatabaseMigrationError` - Migration failures

**Configuration Errors:**
- `ConfigurationError` - Config file/env var issues
...
import { GitHubAuthenticationError, GitHubResourceNotFoundError } from '../../domain/errors';

throw new GitHubAuthenticationError('Invalid token provided');
throw new GitHubResourceNotFoundError('repository', 'web-app');
```
...
if (!config.github.token) {
  throw new ConfigurationError('GitHub token missing', 'github.token');
}

// 2. Wrap third-party errors
try {
  ...
  throw new GitHubApiError('Failed to fetch repository', 500, error);
}

// 3. Use instanceof for error handling
try {
  await syncData();
} catch (error) {
  if (error instanceof GitHubRateLimitError) {
    console.log(`Rate limit. Resets at ${error.resetTime}`);
  } else if (error instanceof ConfigurationError) {
    console.error(`Config error: ${error.configKey}`);
  }
}
...
if (isAppError(error)) {
  console.error(getUserMessage(error));
}
```

#### When Adding New Features:

1. **Check existing error types** - Use appropriate existing error
2. **Create new error type if needed** - Add to `src/domain/errors/index.ts`
3. **Include metadata** - Store relevant context (repo name, field names, etc.)
4. **Update tests** - Add tests for new error types
5. **Update CLI** - Handle new errors in `src/presentation/cli/index.ts`

#### Testing Errors:

```typescript
import { GitHubAuthenticationError } from '../../domain/errors';

it('throws authentication error for invalid token', async () => {
  await expect(client.fetchRepos()).rejects.toThrow(GitHubAuthenticationError);
});

it('includes resource details in error', () => {
...
**⚠️ CRITICAL: Never use `throw new Error()` - Always use custom error types!**
...
### ✅ Via Code/Testing:
4. **All quality checks pass**: `pnpm check` must be green
   - Type checking: `pnpm typecheck`
   - Linting: `pnpm lint`
   - Formatting: `pnpm format:check`
5. **Tests pass**: `pnpm test`
6. **No `any` types**: Use specific types or `unknown`
7. **Zod validation**: Used where runtime validation is needed
8. **Documentation updated**: If needed
9. **Self-review complete**

⚠️ **NEVER mark a task as Done if `pnpm check` fails**

---

## Quick Setup Reference

### Initial Setup (task-001)
```bash
pnpm init
pnpm add typescript @types/node tsx zod
pnpm add -D eslint prettier @typescript-eslint/parser @typescript-eslint/eslint-plugin vitest
```
